...
class decoder(nn.Module):
    def __init__(self, dim, nc=1, skip=True):
        super(decoder, self).__init__()
        self.dim = dim
        nf = 64
        self.skip = skip
        sk_fct = 2 if skip else 1
        ...
        # state size. (nf*8) x 4 x 4
        self.upc2 = dcgan_upconv(nf * 8 * sk_fct, nf * 4)
        # state size. (nf*4) x 8 x 8
        self.upc3 = dcgan_upconv(nf * 4 * sk_fct, nf * 2)
        # state size. (nf*2) x 16 x 16
        self.upc4 = dcgan_upconv(nf * 2 * sk_fct, nf)
        # state size. (nf) x 32 x 32
        self.upc5 = nn.Sequential(
                nn.ConvTranspose2d(nf * sk_fct, nc, 4, 2, 1),
                nn.Sigmoid()
